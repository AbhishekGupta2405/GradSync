...
import { jobAPI, eventsAPI, postAPI, userAPI } from '@/lib/api'

export default function AlumniDashboard() {
  const { user } = useAuth()
  const [isProfileModalOpen, setIsProfileModalOpen] = useState(false)
  const [globalStats, setGlobalStats] = useState({ totalUsers: 0, totalJobs: 0, totalEvents: 0 })
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        const [jobsData, eventsData, feedData, directoryData] = await Promise.all([
          jobAPI.getJobs(),
          eventsAPI.getEvents(),
          postAPI.getFeed(),
          userAPI.getDirectory() // Added back for users count
        ]);
        ...
        if (Array.isArray(jobsData)) setJobOpportunities(jobsData.slice(0, 3));
  ...
  const stats = [
    {
      icon: Users,
      label: 'GradSync Network',
      value: globalStats?.totalUsers || '...',
      change: 'Live connection',
    ...
    },
    {
      ...
      change: 'Global jobs posted',
      color: 'text-golden-500',
      bgColor: 'bg-golden-50'
    },
    {
      icon: MessageCircle,
      label: 'Mentorship Base',
      value: 'Active',
      change: 'You are available',
            ...
            <div className="space-y-6">
              ...
              >
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  <Link to="/alumni/directory" className="flex flex-col items-center p-4 rounded-lg border-2 border-dashed border-gray-200 hover:border-primary-300 hover:bg-primary-50 transition-colors group">
                  ...
                  <Link to="/alumni/jobs" className="flex flex-col items-center p-4 rounded-lg border-2 border-dashed border-gray-200 hover:border-golden-300 hover:bg-golden-50 transition-colors group">
                  ...
                  <Link to="/alumni/events" className="flex flex-col items-center p-4 rounded-lg border-2 border-dashed border-gray-200 hover:border-purple-300 hover:bg-purple-50 transition-colors group">
              ...
              >
                <div className="flex items-center justify-between mb-4">
                  ...
                  <Link to="/alumni/feed" className="text-primary-600 hover:text-primary-700 text-sm font-medium">
                    View All
                  </Link>
                </div>
              ...
              >
                <div className="flex items-center justify-between mb-4">
                  ...
                  <Link to="/alumni/jobs" className="text-primary-600 hover:text-primary-700 text-sm font-medium">
                    View All Jobs
                  </Link>
                </div>
                <div className="space-y-4">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex items-center space-x-4 mt-2 text-sm text-gray-600">
                            ...
                            <span className="bg-primary-100 text-primary-700 px-2 py-1 rounded-full text-xs">
                              {job.experienceLevel || 'Professional'}
                            </span>
                          </div>
                          <p className="text-xs text-gray-500 mt-2">
                            {job.salary ? `Salary: ${job.salary}` : 'Competitive Salary'} • Deadline: {job.deadline || 'Rolling'}
                          </p>
                        </div>
                        <Link to="/alumni/jobs" className="btn-primary text-sm px-4 py-2 flex items-center space-x-1">
                          <span>Apply</span>
