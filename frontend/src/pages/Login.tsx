...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Compute dashboard destination cleanly by checking the registered localStorage state after auth context resolution
      let targetRole = 'student'
      const rawMem = localStorage.getItem('gradsync_user')
      if (rawMem) {
        try { targetRole = JSON.parse(rawMem).role.toLowerCase() || 'student' } catch { /* ignore parse errors */ } 
      }
      
      router(`/${targetRole}/dashboard`)
    } catch (error: any) {
      ...
      >
        ...
        >
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full btn-primary py-3 text-lg disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center"
          >
            {isLoading ? (
              <>
