...
function replaceNextSpecifics(content) {
    ...
    result = result.replace(/router\.push\(/g, 'router(');
    return result;
}
...
function copyConfigs() {
    try {
        fs.copyFileSync('D:\\\\gradsync\\\\tailwind.config.js', 'D:\\\\Gradsync2.0\\\\frontend\\\\tailwind.config.js');
        fs.copyFileSync('D:\\\\gradsync\\\\postcss.config.js', 'D:\\\\Gradsync2.0\\\\frontend\\\\postcss.config.js');
        console.log("Configs copied");
    } catch(e) {
        console.error("Config copy error", e);
    }
}
